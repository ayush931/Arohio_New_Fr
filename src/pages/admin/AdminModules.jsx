...
export default function AdminModules() {
    ...
    const [form, setForm] = useState({
        code: MODULE_CODE_OPTIONS[0].value,
        name: MODULE_CODE_OPTIONS[0].label,
        description: "",
        is_active: true,
    });
    ...
    const API = `${API_BASE}/api/v1/modules`;
    ...
    const openEdit = (mod) => {
        setEditing(mod);
        // If existing code isn't one of the fixed 4, default to first
        const safeCode = MODULE_CODE_OPTIONS.some(o => o.value === mod.code) ? mod.code : MODULE_CODE_OPTIONS[0].value;
        setForm({
            code: safeCode,
            name: mod.name || "",
    ...
    const onChangeCode = (newCode) => {
        const opt = MODULE_CODE_OPTIONS.find(o => o.value === newCode) || MODULE_CODE_OPTIONS[0];
        // Auto-fill name to the label of the selected code (you can edit name if you want)
    ...
    const softDelete = async (id) => {
        if (!confirm("Delete this module? You can restore via API if needed.")) return;
        try {
    ...
    return (
      ...
      `}</style>
            ...
            <h1 className="h5 title mb-1">Modules Management</h1>
            <div className="muted mb-3">Create, edit, activate/deactivate and delete modules.</div>
            ...
            <div className="toolbar">
                ...
                <button className="btn-teal" onClick={openCreate}>+ New Module</button>
            </div>
                ...
                <table className="table">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan={8} style={{ padding: 18 }} className="muted">No modules found.</td>
                            </tr>
                        ) : (
                            items.map((m) => (
                                <tr key={m.id}>
                                    <td>{m.id}</td>
                                    <td style={{ fontWeight: 800, color: INK }}>{m.code}</td>
                                    <td style={{ fontWeight: 800, color: INK }}>{m.name}</td>
                                    <td className="muted">{m.description || "—"}</td>
                                    <td>
                                        <span className={`pill ${m.is_active ? "a-yes" : "a-no"}`}>
                                    ...
                                    <td>
                                        <div className="actions">
                                            ...
                                            <button
                                                ...
                                                data-tip={m.is_active ? "Deactivate" : "Activate"}
                                                onClick={() => toggleActive(m.id)}
                                                aria-label={m.is_active ? "Deactivate module" : "Activate module"}
                                            >
                                                {m.is_active ? <ToggleOnIcon /> : <ToggleOffIcon />}
                                            </button>
                                            <button
                                                className="icon-btn"
                                                title="Delete"
                ...
                <div className="modal-backdrop" role="dialog" aria-modal="true">
                    <div className="modal-card">
                        <div className="modal-head">
                            ...
                            <div style={{ fontWeight: 900, color: INK }}>{editing ? "Edit Module" : "Create Module"}</div>
                        </div>

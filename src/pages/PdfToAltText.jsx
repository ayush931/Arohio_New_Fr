...
/* ---------- API base & prefix (moved up so helpers can use them) ---------- */
const API_BASE =
  (import.meta?.env?.VITE_API_BASE && import.meta.env.VITE_API_BASE.replace(/\/+$/, "")) ||
  "http://localhost:8000";
const API_PREFIX = `${API_BASE}/uploads-pdf-images`;

/* Save JSON via backend (new prefix) */
...
/* Fetch server manifest (new prefix) */
...
/* ---------- Component ---------- */
export default function PdfToAltText() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_PREFIX}/${encodeURIComponent(userId)}/projects`, {
      headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) },
    })
      .then(async (r) => {
  ...
  const onSubmit = async () => {
    ...
    const uploadUrl = extractRaw
      ? `${API_PREFIX}/${encodeURIComponent(userId)}/raw`
      : `${API_PREFIX}/${encodeURIComponent(userId)}`;
  ...
  const onGenerateAltText = async () => {
    ...
    try {
      const res = await fetch(
        `${API_PREFIX}/${encodeURIComponent(userId)}/update-visibility`,
        {
          method: "POST",
          headers: {
  ...
  async function handleCreateProjectInline() {
    ...
    try {
      const res = await fetch(
        `${API_PREFIX}/${encodeURIComponent(userId)}/projects`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const onExport = async (formatKey) => {
    ...
    const url = `${API_PREFIX}/${encodeURIComponent(userId)}/export`;
          ...
          <div className="row g-3">
            ...
            <div className="col-12 col-lg-3">
              <div className="border rounded p-3">
                ...
                <ul className="text-muted small ps-3 mb-0" style={{ lineHeight: 1.7 }}>
                  <li>Please check each image carefully before import.</li>
                  <li>Do not select irrelevant or duplicate images.</li>
                  <li>Unnecessary images increase processing load.</li>
                  <li>Keep only required visuals for best performance.</li>
                </ul>
          ...
          {!file ? (
            <div className="d-flex align-items-center justify-content-center">
              <button
                ...
                style={{ borderColor: brand, color: brand, transition: "all 0.2s ease" }}
                onMouseEnter={(e) => {
            ...
            <div className="d-flex flex-column align-items-center">
              ...
              <div className="mt-4 p-3 border text-start mx-auto" style={{ maxWidth: 450, borderRadius: 10, background: "#f8fafc", borderColor: "#e2e8f0" }}>
                <div className="fw-bold mb-2 text-dark" style={{ fontSize: "0.9rem" }}>Image Extraction Mode</div>
                <div className="form-check mb-2">
                  <input
                    ...
                    style={{ cursor: "pointer" }}
                  />
                  <label className="form-check-label text-dark small" htmlFor="modeSmart" style={{ cursor: "pointer" }}>
                    <strong>Smart Bounding Boxes</strong> — Detects and crops visual regions (charts, diagrams, tables, figures).
                  </label>
                </div>
                <div className="form-check">
                  <input
                    ...
                    style={{ cursor: "pointer" }}
                  />
                  <label className="form-check-label text-dark small" htmlFor="modeRaw" style={{ cursor: "pointer" }}>
                    <strong>Raw Image Extraction</strong> — Extracts exact embedded images directly from the PDF without cropping.
                  </label>
                </div>
              </div>
